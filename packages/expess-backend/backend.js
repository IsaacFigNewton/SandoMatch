...
try {
    ...
    console.error("Error reading formatted sandwiches file:", err);
}
...
app.get("/sandwiches/random", (req, res) => {
    const randomIndex = Math.floor(Math.random() * formattedSandwiches.length);
...
app.get("/sandwiches/:id", (req, res) => {
    ...
    const sandwich = sandwichesList.find(sandwich => sandwich.id === id);
