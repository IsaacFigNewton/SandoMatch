const findSandwichById = (sandwichesList, id) => {
...
const filterSandwiches = (sandwichesList, filters) => {
  ...
  if (ingredient) {
    ...
    filteredSandwiches = filteredSandwiches.filter((sandwich) =>
      Object.values(sandwich.ingredients || {}).some(
        (category) =>
          Object.values(category)
            .flat()
            .some(
              (item) =>
                item.toLowerCase() === lowerCaseIngredient
            )
      )
    );
  }
  ...
  if (maxCost) {
    filteredSandwiches = filteredSandwiches.filter(
      (sandwich) =>
        sandwich.costs &&
        sandwich.costs[0] <= parseFloat(maxCost)
    );
  }
  ...
  if (minCalories || maxCalories) {
    filteredSandwiches = filteredSandwiches.filter(
      (sandwich) => {
        const calories =
          sandwich.calories && sandwich.calories[0];
        return (
