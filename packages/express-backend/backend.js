...
const formattedSandwichesPath = path.resolve(
  __dirname,
  "../react-frontend/public/sandwich-dataset/formatted_sandwich_entries.json"
);

let costCalEstimates = {};
let formattedSandwiches = {};
let restaurantIngredientsList = [];
let sandwichesList = [];

// load the cost and calorie estimates from the JSON file
try {
  const data = fs.readFileSync(costCalEstimatePath, "utf8");
...
// load formatted sandwiches from JSON file
try {
  const data = fs.readFileSync(formattedSandwichesPath, "utf8");
  formattedSandwiches = JSON.parse(data);
} catch (err) {
  console.error(
    "Error reading formatted sandwiches file:",
    err
  );
}
...
app.get("/sandwiches/vegan", (req, res) => {
  ...
  const veganSandwich = {
    id_: formattedSandwiches.length,
    name: `Vegan Sandwich ${formattedSandwiches.length + 1}`,
  ...
  formattedSandwiches.push(veganSandwich);
...
app.get("/sandwiches/filter", (req, res) => {
  ...
  let filteredSandwiches = formattedSandwiches;
...
app.get("/sandwiches/random", (req, res) => {
  const randomIndex = Math.floor(
    Math.random() * formattedSandwiches.length
  );
  const randomSandwich = formattedSandwiches[randomIndex];
  if (randomSandwich) {
...
// retrieve all generated sandwiches
app.get("/sandwiches", (req, res) => {
  res.json(sandwichesList);
});
...
app.get("/sandwiches/:id", (req, res) => {
  ...
  const sandwich = sandwichesList.find(
    (sandwich) => sandwich.id === id
  );

  if (!sandwich) {
    return res
      .status(404)
      .json({ error: "Sandwich not found." });
  }
...
const findExistingSandwich = (ingredients, restaurantId) => {
  ...
  return sandwichesList.find((sandwich) => {
    console.log(
      "Comparing with Existing Sandwich:",
      JSON.stringify(sandwich.ingredients, null, 2)
    );
    return (
      sandwich.restaurant === restaurantId &&
